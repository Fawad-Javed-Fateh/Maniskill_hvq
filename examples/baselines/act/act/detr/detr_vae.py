...
class ACT_DETRVAE(nn.Module):
    ...
    def forward(self, obs, actions=None):
        ...
        if is_training:
            ...
            # encoder_output=out['q_q']
            # vq_vae_loss=out['loss']
            encoder_output, vq_vae_loss = self.encoder(encoder_input)
            
            latent_input = encoder_output[0] # take cls output only
            # print(encoder_output.data.shape)
            # latent_info = self.latent_proj(latent_input)
            # mu = latent_info[:, :self.latent_dim]
            # logvar = latent_info[:, self.latent_dim:]
            # latent_sample = reparametrize(mu, logvar)
            # latent_input = self.latent_out_proj(latent_sample)
            # print(latent_input.data.shape)
        else:
            # mu = logvar = None
            # latent_sample = torch.zeros([bs, self.latent_dim], dtype=torch.float32).to(state.device)
            latent_input = torch.zeros([bs, self.hidden_dim]).to(state.device)
        ...
        a_hat = self.action_head(hs)
        return a_hat, vq_vae_loss
    

class MCR_DETRVAE(nn.Module):
    """ This is the DETR module that performs object detection """
    def __init__(self, backbones, transformer, encoder, state_dim, action_dim, num_queries):
        super().__init__()
        self.num_queries = num_queries
