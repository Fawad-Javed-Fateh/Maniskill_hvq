...
class TransformerBackbone(nn.Module):
    def __init__(self, embed_dim, num_heads, depth, mlp_dim):
        super().__init__()
...
class Actor(nn.Module):
    def __init__(self, state_dim, action_dim, embed_dim=256, num_heads=1, depth=4, mlp_dim=512):
        super().__init__()
        self.encoder = PlainConv(
            in_channels=4, out_dim=embed_dim, max_pooling=False, inactivated_output=False
        )
        self.hidden_mapper = nn.Linear(embed_dim + state_dim, embed_dim * 2)
        self.transformer = TransformerBackbone(
            embed_dim=embed_dim * 2, num_heads=num_heads, depth=depth, mlp_dim=mlp_dim
        )
    ...
    def forward(self, rgbd, state):
        ...
        feature = self.encoder(img)  # (B, embed_dim)
        x = torch.cat([feature, state], dim=1)  # (B, embed_dim + state_dim)
        x = self.hidden_mapper(x).unsqueeze(1)  # (B, 1, embed_dim * 2)
        x = self.transformer(x).squeeze(1)  # (B, embed_dim * 2)
